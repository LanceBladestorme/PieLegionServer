...
def __toonsMiss(attack, level, hp):
    return __doToonsMiss(attack, level, hp)
	
	
def __cogsHit(attack, level, hp):
    return __doCogsHit(attack, level, hp)		


NPCSOSfn_dict = {ToontownBattleGlobals.NPC_COGS_MISS: __cogsMiss,
 ToontownBattleGlobals.NPC_TOONS_HIT: __toonsHit,
 ToontownBattleGlobals.NPC_TOONS_MISS: __toonsMiss,
 ToontownBattleGlobals.NPC_COGS_HIT: __cogsHit,
...
def __doNPCSOS(sos):
    ...
    if track != None:
        ...
        return __cogsMiss(sos, 0, 0) or __cogsHit(sos, 0, 0)
    return
...
def __doToonsMiss(attack, level, hp):
    track = __doSprinkle(attack, 'toons', hp)
    pbpText = attack['playByPlayText']
    pbpTrack = pbpText.getShowInterval(TTLocalizer.MovieNPCSOSToonsMiss, track.getDuration())
    return (track, pbpTrack)
...
def __doCogsHit(attack, level, hp):
    track = __doSprinkle(attack, 'suits', hp)
    pbpText = attack['playByPlayText']
    pbpTrack = pbpText.getShowInterval(TTLocalizer.MovieNPCSOSCogsHit, track.getDuration())
    return (track, pbpTrack)
