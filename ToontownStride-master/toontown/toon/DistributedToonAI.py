...
@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[int, int])
...
@magicWord(category=CATEGORY_SYSTEM_ADMINISTRATOR, types=[str, int, int])
...
def trackBonus(trackIndex, level):
    """
    Modifies the target's track bonus level.
    """
    target = spellbook.getTarget()
    ...
    return 'Target track bonus level has been set to %s!' % level

@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[str, str, int])
def track(command, track, value=None):
    ...
    target = spellbook.getTarget()
    trackAccess = target.getTrackAccess()
    ...
    if command.lower() == 'remove':
        ...
        trackAccess[index] = 0
        target.b_setTrackAccess(trackAccess)
...
@magicWord(category=CATEGORY_COMMUNITY_MANAGER, types=[int, str])
